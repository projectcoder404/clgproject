...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_reference_book'])) {
        ...
        $reference_books = $_POST['reference_book'];
        ...
        try {
            ...
            foreach($reference_books as $book) {
                ...
                $stmt->bind_param("ss", $course_code, $book);
                $stmt->execute();
            }
            
            $conn->commit();
            $_SESSION['message'] = "Reference books added successfully!";
    ...
    if (isset($_POST['update_reference_book'])) {
        ...
        try {
            ...
            $delete_stmt = $conn->prepare("DELETE FROM reference_book WHERE course_code = ?");
            $delete_stmt->bind_param("s", $course_code);
            $delete_stmt->execute();
            
            $insert_stmt = $conn->prepare("INSERT INTO reference_book (course_code, reference_book) VALUES (?, ?)");
            foreach($reference_books as $book) {
                $book = $conn->real_escape_string($book);
                $insert_stmt->bind_param("ss", $course_code, $book);
                $insert_stmt->execute();
            }
            
            $conn->commit();
            $_SESSION['message'] = "Reference books updated successfully!";
            $_SESSION['message_type'] = "success";
        } catch (Exception $e) {
            $conn->rollback();
            $_SESSION['message'] = "Error: " . $e->getMessage();
    ...
    if (isset($_POST['delete_reference_book'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM reference_book WHERE course_code = ?");
        $stmt->bind_param("s", $course_code);
        if ($stmt->execute()) {
            $_SESSION['message'] = "Reference books deleted successfully!";
            $_SESSION['message_type'] = "success";
        }
        $stmt->close();
        header("Location: ".$_SERVER['PHP_SELF']);
...
$result = $conn->query("SELECT * FROM reference_book ORDER BY id DESC");
$grouped_books = [];
...
<body>
    ...
    <div class="content-area">
        <button class="btn btn-primary add_items" onclick="openModal()">
            <i class="fas fa-plus"></i> Add New Reference Books
        </button>
                            ...
                            <td>
                                <ul style="margin: 0; padding-left: 1.5rem;">
                                    <?php foreach ($books as $book): ?>
                                        <li><?= htmlspecialchars($book['reference_book']) ?></li>
                                    <?php endforeach; ?>
                                </ul>
                            </td>
                            <td class="action">
                                <button class="btn btn-primary" onclick="openEditModal(
                                    ...
                                    <?= htmlspecialchars(json_encode(array_column($books, 'reference_book'))) ?>
                                )">
            ...
            <div class="modal-content">
                ...
                <form method="POST">
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" name="update_reference_book" class="btn btn-success">
                            <i class="fas fa-save"></i> Save All
                        </button>
                    </div>
                </form>
        ...
        <div id="deleteModal" class="modal">
            <div class="modal-content">
                ...
                <p>Are you sure you want to delete all reference books for this course?</p>
                <form method="POST">
    ...
    <script>
        ...
        function openEditModal(course_code, books) {
            ...
            books.forEach((book, index) => {
                ...
                bookGroup.innerHTML = `
                    ...
                    <div class="form-row">
                        <div class="form-column">
                            <label>Reference Book</label>
                            <input type="text" name="reference_book[]" value="${book}" required>
                        </div>
                    </div>
                `;
